...
class VerifierAndConverterMethodTest {
    ...
    static class Cmd implements Runnable {
        @Option(names = "--port", converterMethod = "me.bechberger.minicli.VerifierAndConverterMethodTest$Helpers#parsePort",
                verifierMethod = "me.bechberger.minicli.VerifierAndConverterMethodTest$Helpers#verifyPort")
        int port;
    ...
    void optionConverterAndVerifierMethodWork() {
        Cmd cmd = new Cmd();
        RunResult res = MiniCli.builder().runCaptured(cmd, "--port", "123", "5");
    ...
    void verifierFailureIsUsageError() {
        Cmd cmd = new Cmd();
        RunResult res = MiniCli.builder().runCaptured(cmd, "--port", "70000", "5");
