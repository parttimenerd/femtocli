...
public class BooleanOptionConverterTest {
    ...
    void parsesBooleanWithEquals() {
        Cmd cmd = new Cmd();
        RunResult res = MiniCli.builder().runCaptured(cmd, "--turn=on");
    ...
    void parsesBooleanWithSeparateToken() {
        Cmd cmd = new Cmd();
        RunResult res = MiniCli.builder().runCaptured(cmd, "--turn", "off");
        assertEquals(0, res.exitCode(), res.err());
    ...
    void missingValueFails() {
        Cmd cmd = new Cmd();
        RunResult res = MiniCli.builder().runCaptured(cmd, "--turn");
