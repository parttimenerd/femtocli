...
public class EnumTest {
    ...
    public void testEnumWithoutDescription_Help() {
        ...
        // Should show simple enum names without descriptions (lowercase)
    ...
    public void testEnumMissingDescription_HelpShouldFail() {
        // showEnumDescriptions=true but enum constants have no @Description.
        // The processor falls back to just showing the constant names (no exception).
        var res = FemtoCli.runCaptured(new EnumMissingDescriptionExample(), "--help");
        assertEquals(0, res.exitCode());
        assertTrue(res.out().contains("active, inactive"), () -> "Expected enum names in help, got:\n" + res.out());
    }
}